                ...
                loff_t *f_pos)
{
	...
	int response = down_interruptible(&r);
	if(response) {
		return 0;
	}
	...
	/* get to start index in arr */
	void *a = arr;
	int i;
	for(i = 0; i < start; i++) {
		a += sizeof(size_t) + scull_fifo_elemsz;
	}
	/* get len from arr */
	size_t len = *((size_t*)a);
	...
	int c;
	/* copy the str to user space */
	if(count <= len) {
		c = copy_to_user(buf, str, count);
	}
	else {
		c = copy_to_user(buf, str, len);
	}

	/* if unable to copy to user_space, err */
	if(c != 0) {
		printk(KERN_NOTICE "Error copying to user space.");
	}

	/* if at end of arr, reset start */
	if(start == scull_fifo_size - 1) {
		start = 0;
	}
...
{
	/* down write */
	int response = down_interruptible(&w);
	if(response) { 
		return 0;
	}
	/* lock mutex */
	...
	if(count > scull_fifo_elemsz) {
		c = copy_from_user(str, buf, scull_fifo_elemsz);
		/* set len */
		*len = scull_fifo_elemsz;
	}
	else {
		c = copy_from_user(str, buf, count);
		/* set len */
		*len = count;
	}

	if(c != 0) {
		printk(KERN_NOTICE "Error copying from user space.");
	}

	/* if at end of array, reset end */
	if(end == scull_fifo_size - 1) {
		end = 0;
	}
	/* increment end's index */
	else {
		end++;
	}
...
int scull_init_module(void)
{
	...
	sema_init(&r, 0);
	sema_init(&w, scull_fifo_size);
	/* kzalloc arr to get index starting at bit 0 */
	/* basically n indexes * sizeof each index */
	arr = kzalloc(scull_fifo_size*(scull_fifo_elemsz + sizeof(size_t)), GFP_KERNEL);
	if(!arr) {
		printk(KERN_NOTICE "Error mallocing array.");
	}
	/* initialize start and end to index 0 */
	start = 0;
	end = 0;
